...
var IpcProvider = SandboxedModule.require('../lib/web3_hpb/ipcprovider', {
    requires: {
...
describe('lib/web3_hpb/ipcprovider', function () {
    describe('send', function () {
        it('should send basic request', function () {
            var provider = new IpcProvider('', net);
            var result = provider.send({id: 1, method: 'hpb_test'});
    ...
    describe('sendAsync', function () {
        it('should send basic async request', function (done) {
            var provider = new IpcProvider('', net);
            provider.sendAsync({id: 1, method: 'hpb_test'}, function (err, result) {
                assert.isObject(result);
                done();
            });
        }); 
